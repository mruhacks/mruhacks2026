...
export default async function RegisterEventPage({ params }: Props) {
  ...
  const [previousRegistration, options, profileResult] = await Promise.all([
  ...
  const prev = previousRegistration.success ? previousRegistration.data : null;
  ...
  if (!hasProfile && !previousRegistration.success) {
    redirect(`/dashboard/profile?next=/dashboard/register/${eventId}`);
  }
